...
import {InputNumber, Input, Switch, Select, Form} from 'antd';
...
const CreateEvent = (props: CreateEventProps) => {
	...
	const [limitAttendees, setLimitAttendees] = useState<number | undefined>(null);
	const [visibility, setVisibility] = useState(true);
	const [invitees, setInvitees] = useState<string[]>([]);
	const [hideFrom, sethideFrom] = useState<string[]>([]);
	const [imageSelected, setImageSelected] = useState<File | undefined>(null);
	const [tempImageUrl, setTempImageUrl] = useState<string | undefined>(null);
	...
	function handleMapSelect(newCoordinates: L.LatLngExpression) {
	...
	async function photoUpload(file: File) {
		...
		await sendPictureToCloud(formData)
			.then((data: CloudinaryResponse) => data.publicId);
	}
	...
	const handleLocationSelect = (e: string) => {
		...
		geocodeByAddress(e)
			...
			.then(async (results: any) => getLatLng(results[0]))
			.then((latLng: any) => {
				console.log(latLng);
	...
	const handleFormSubmit = async () => {
		...
		if (imageSelected) {
			await photoUpload(imageSelected);
		}
	...
	return (
		...
		>
			...
			<div className='modal-form-wrapper'>
				{!isLoading && <>
					{step === 0 && <>
						<Form name='create-event-first' onFinish={() => {
							handleStep(true);
						}}>
					...
					<Form
						...
						<Form.Item name='limitAttendees' label='Limit Attendees'>
							<InputNumber
								...
								onChange={(numberOfAttendees: number | undefined): void => {
									setLimitAttendees(numberOfAttendees);
								}}
							/>
						</Form.Item>
    	...
    	<Form
    		...
    		<div className='preview-details'>
    			<div className='event-details'>
    				<p>{activeUser ? activeUser.username : 'No User'}</p>
